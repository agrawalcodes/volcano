...
var _ = Describe("Agent Scheduler E2E Test", func() {
	...
	// Batch throughput needs multiple feasible nodes. In hard mode the agent
	// shard is capped to a single node (node-limit maxNodes=1), so a batch
	// large enough to exceed one node's capacity cannot be placed and the test
	// is flaky. Soft/none modes allow fallback onto the volcano shard nodes, so
	// the batch spreads and the throughput assertion is meaningful.
